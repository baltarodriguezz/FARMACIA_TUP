...
if (perfilForm) {
  perfilForm.addEventListener("submit", async (e) => {
    ...
    const clienteNombre = sessionStorage.getItem("clienteNombre"); // o como lo guardaste

// Nombre arriba (ya lo hacías)
const userNameSpan = document.getElementById("userName"); // ese que dice "Hola, balta"
if (userNameSpan) userNameSpan.textContent = clienteNombre;

// ➕ Nombre en el menú (lo nuevo)
const nombreUsuarioMenu = document.getElementById("nombreUsuarioMenu");
if (nombreUsuarioMenu) nombreUsuarioMenu.textContent = clienteNombre;
...
// 1. LÓGICA DEL BOTÓN DE ÍCONO DE USUARIO (el que tiene id="inicio_sesion")
 if (inicioSesionBtn) {
    ...
    if (btnAbrirLoginDesdeMenu) {
    btnAbrirLoginDesdeMenu.addEventListener("click", (e) => {
      e.stopPropagation();
      if (userMenuNoSesion) cerrarMenu(userMenuNoSesion);
      abrirLoginModal();
    });
  }
...
if (
  ...
  if (cerrarSesionPerfilBtn) {
    cerrarSesionPerfilBtn.addEventListener("click", () => {
      ...
      localStorage.removeItem("userRole");
      isLoggedIn = false;
      // Acá normalmente redirigís desde el HTML
    });
  }
});
